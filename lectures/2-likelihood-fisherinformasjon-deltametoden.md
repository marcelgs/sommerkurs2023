...
$\defnn{\text{Maximum likelihood-estimatoren (MLE)}}{\hat{\theta}_\text{MLE}} = \underset{\theta \in \Theta}{\arg\max}\  \mathcal{L}_n(\theta) = \underset{\theta \in \Theta}{\arg\max}\ \mathcal{\ell}_n(\theta)$
...
$$\deldel{\Sigma^{-1}} \ell_n (\bm\mu, \Sigma)  = 0 \Rightarrow \Sigma = \frac 1 n \sum_{i=1}^n (\bm x_i-\bm\mu)(\bm x_i -\bm\mu)^T := \hat\Sigma$$
